...
export function authorize(action: ProposedAction, context: PolicyContext): AuthorizationDecision {
  ...
  if (url === null) {
    return deny('url', 'The action targets a URL that could not be parsed.');
  }
  ...
  if (origin === undefined) {
    return deny('origin', 'The origin is not in the allowlist.');
  }

  const path = url.pathname;
  if (origin.deniedPaths.some((glob) => globToRegExp(glob).test(path))) {
    return deny('deniedPath', 'The path is denied by the allowlist.');
  }
  if (!origin.allowedPaths.some((glob) => globToRegExp(glob).test(path))) {
    return deny('path', 'No allowed path pattern covers this path.');
  }
