...
**Amendment, 2026-09-13. Toolchain versions, pinned exactly before the first line of code.** TypeScript 6.0.3, Vitest 4.1.11 and `@types/node` 22.20.2, written as exact versions in `package.json` with no caret ranges. The newest majors were rejected on evidence rather than caution. TypeScript 7.0.2, the native compiler, sits outside the range typescript-eslint 8.70.0 declares as supported, which stops below 6.1, so choosing it would have broken the lint task planned at the time. Vitest 5.0.0 was ten days old when this was decided, and a reviewer running a clean clone should not be the first person to find a defect in a new major of the test runner. The cost is that both majors are adopted later as a deliberate upgrade with its own commit rather than for free now. Zod is pinned when it lands at S1-T02.

Vite is pinned at 7.3.6 and forced there on every dependency edge with an npm `overrides` entry, and that part was found rather than chosen. With npm 10.9.2, installing Vitest 4.1.11 on a clean tree crashed inside the resolver on a null reference instead of reporting a conflict. The cause is a chain. Vitest accepts Vite 6, 7 or 8, so npm considers Vite 8.3.0, which declares an optional peer on `@vitejs/devtools`. The devtools Vitest integration declares `vitest` as a wildcard peer, which since 3 September resolves to Vitest 5.0.0, whose Playwright browser package demands exactly 5.0.0 and collides with the pinned 4.1.11. Forcing Vite to 7.3.6, which declares no devtools peer, stops the chain at its source, and was verified in an isolated install before being applied here. Forcing Vitest to 4.1.11 everywhere also installed, but it left a three day old Vite 8.3.0 in the tree and would mask any later package that genuinely needs a different Vitest, so it was rejected. The consequence worth recording is that this failure depended on the calendar. The same pinned install worked before Vitest 5.0.0 was published, so exact pins on direct dependencies do not make an install reproducible on their own. The committed lockfile does.
