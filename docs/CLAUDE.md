...
Operating manual for this repository. The root `CLAUDE.md` is a pointer at this file, because the repository root is for deliverables and this is not one. Read this fully at the start of every session, then read `PROGRESS.md` to find out where we are.
...
```
CLAUDE.md              a two line pointer at this file, because the root is for deliverables
...
docs/PROGRESS.md       living state, updated once per slice
