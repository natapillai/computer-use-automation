...
Four replay runs are committed to `/evidence/replay/`. The success, because it is the thread. `MEMBER_NOT_FOUND`, because it proves the business outcome split, which the brief calls the most common design mistake. And `surpriseDialog`, because it proves the system stops rather than clicking through something it does not understand. Nobody claimed that one, so it ends unclaimed and the capture shows the dialog still open with its OK button never pressed. The handoffs a person actually took are in the discovery runs, which is where they happened. The fourth is a hard failure, kept because a structured result says what was expected and what was observed and only the capture shows the page that produced it.
