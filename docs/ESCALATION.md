...
* Interventions live in an in memory store with no journal. A run owns its browser, so a process restart loses the session the intervention pointed at, which makes a restored claim worse than no claim. The trace, the transcript and the captures are written as events happen, so a run that is killed while it waits still leaves what it had done. The manifest is not, because it summarises a run that finished, and a killed run has no summary to write. A real deployment needs durable storage and a session that outlives one process, and `InterventionStore` is the seam for the first half.
