...
* [x] **S7-T02** JSON Schema generation and the review sheet.
  ...
  * Note: both are written next to the artifact on every write, because a sheet nobody can find is a function with a test, and a sheet describing a version other than the one beside it is worse than none. Approving was writing only the JSON until the first approval of a write capability left a sheet saying draft beside an approved artifact, which is the failure this note warns about. Approving now rewrites both. The artifact schema is committed at `docs/capability.schema.json` rather than written into `capabilities/`, because Zod emits `Number.MAX_SAFE_INTEGER` as an integer bound and seventeen digits trip the evidence scanner's account number rule. A test fails if the committed copy and the generated one differ.
