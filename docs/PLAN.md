...
* [ ] **S3-T01** `SurfaceDriver` contract suite, run against the fake and the web driver.
  ...
  * Test: both drivers pass stale token rejection, `not_found` and `ambiguous` resolution, frame path scoping, a derived label on an unnamed node, a geometric relation, and a bounded `waitFor` timeout that names its condition.
